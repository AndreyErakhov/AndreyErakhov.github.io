...
const Gallery = (props) => {
    ...
    useEffect(() => {
        const getApi = async () => {
            try {
                ...
                const resp = await axios.get(`https://test-front.framework.team/paintings`)
                props.setGallery(resp.data)
                props.setPageNumbers(props.createPageNum(resp.data.length,props.limit))
    ...
    function pageNumberPaginate(page, pageNumbers) {
        ...
        if(page === 1) {
            ...
            if(pageNumbers.length < 3) {
                ...
                setArrPageNumbers( [pageNumbers.length - 2, pageNumbers.length - 1, pageNumbers.length])
            }
        }
        else{
    ...
    const currentCountry = props.gallery.slice(firstCountryIndex, lastCountryIndex)
